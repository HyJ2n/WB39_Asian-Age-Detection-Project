...
def preprocess_images(train_path):
    # 캐시 파일 경로 설정
    # 캐시 파일이 존재 할 경우 데이터 전처리 안해도 됨 
    cache_file = 'preprocessed_data.pkl'  
    
    # 이미 캐시 파일이 있는지 확인하고 로드
    if os.path.exists(cache_file):
        print("캐시에서 전처리된 데이터를 로드하는 중")
...
if __name__ == "__main__":
    ...
    train_path = sorted(glob.glob(r"C:\Users\admin\Desktop\test\All-Age-Faces Dataset\original images\*.jpg"))  # 실제 데이터셋 경로에 맞게 설정
